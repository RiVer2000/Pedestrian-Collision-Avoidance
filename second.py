...
class PIDLongitudinalController():
    ...
    def run_step(self, target_speed):
        ...
        return pid_controller(target_speed, current_speed)

    def pip_controller(self, target_speed, current_speed):
...
def main():
    ...
    try:
        ...
        control_vehicles = VehiclePIDController(vehicle, args_lateral={'K_P':1, 'K_D': 0.0, 'K_I':0.0}, args_longitudinal={'K_P':1, 'K_D': 0.0, 'K_I':0.0})
        
        while True:
            waypoints = world.get_map().get_waypoints(vehicle.get_location())
            waypoint = np.random.choice(waypoints(0.3))
            control_signal =  control_signal.run_step(5,waypoint)
